...
void* AiecEA_result_gateway(void* srv_conn, void* arg){
...
#if DEBUG_FLAG
      printf("Results for client %d are all collected in AiecEA_result_gateway, update ready_pool\n", cli_id);
#endif
#if DEBUG_LOG
      char logname[20];
      sprintf(logname, "gateway.log");
      FILE *log = fopen(logname, "a");
      fprintf(log, "Results for client %d, frame %d are all collected in AiecEA_result_gateway, update ready_pool at time %f\n", cli_id, frame_seq, sys_now_in_sec()-start_time);
      fclose(log);
#endif
#if DEBUG_TIMING
...
void AiecEA_collect_result_thread(void *arg){
   const char* request_types[]={"result_gateway"};
   void* (*handlers[])(void*, void*) = {AiecEA_result_gateway};
...
void AiecEA_merge_result_thread(void *arg){
...
void AiecEA_gateway(uint32_t N, uint32_t M, uint32_t fused_layers, char* network, char* weights, uint32_t total_edge_number, const char** addr_list){
   device_ctxt* ctxt = AiecEA_gateway_init(N, M, fused_layers, network, weights, total_edge_number, addr_list);
   sys_thread_t t3 = sys_thread_new("AiecEA_work_stealing_thread", AiecEA_work_stealing_thread, ctxt, 0, 0);
   sys_thread_t t1 = sys_thread_new("AiecEA_collect_result_thread", AiecEA_collect_result_thread, ctxt, 0, 0);
   sys_thread_t t2 = sys_thread_new("AiecEA_merge_result_thread", AiecEA_merge_result_thread, ctxt, 0, 0);
