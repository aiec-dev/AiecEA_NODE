#include "AiecEA_annotation.h"

static AiecEA_annotation_data AiecEA_annot_data[PARTITIONS_H_MAX][PARTITIONS_W_MAX][FUSED_LAYERS_MAX][NUM_OF_FUNCTIONS];
   ...
   if(profile_data == NULL){
      ...
      while(fgets(buffer, BUFFER_SIZE, profile_data) != NULL){
	    ...
	    switch (token_number){
               ...
               case 6: AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].avg_duration[frame_number][partition_number][data_reuse] = atof(token); 
                       AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].valid[frame_number][partition_number][data_reuse] = true;
                       break;
            }
   ...
   for(function_id = 0; function_id < NUM_OF_FUNCTIONS; function_id++){
      for(frame_number = 0; frame_number < FRAME_NUM; frame_number++){
         for(partition_number=0; partition_number<PARTITIONS_MAX; partition_number++){
            AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].valid[frame_number][partition_number][0] = false;
            AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].valid[frame_number][partition_number][1] = false;
            AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].avg_duration[frame_number][partition_number][0] = 0.0;
            AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].avg_duration[frame_number][partition_number][1] = 0.0;
         }
      }
   }
...
void function_delay(char* function_name, device_ctxt* ctxt, void* sim_ctxt, uint32_t frame_number, uint32_t partition_number, uint32_t data_reuse){
   ...
   uint32_t partitions_w = model->ftp_para->partitions_w;
   uint32_t layers = model->ftp_para->fused_layers;
   if(AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].valid[frame_number][partition_number][data_reuse]){
      total_simulation_time = total_simulation_time + AiecEA_annot_data[partitions_h][partitions_w][layers][function_id].avg_duration[frame_number][partition_number][data_reuse];
   }
