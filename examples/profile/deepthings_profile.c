#include "AiecEA_profile.h"

static AiecEA_profile_data AiecEA_prof_data[NUM_OF_FUNCTIONS];
      ...
      for(frame_number = 0; frame_number < FRAME_NUM; frame_number++){
         for(partition_number=0; partition_number<PARTITIONS_MAX; partition_number++){
            for(data_reuse=0; data_reuse<2; data_reuse++){
               if(AiecEA_prof_data[function_id].valid[frame_number][partition_number][data_reuse] == false) continue;
               fprintf(f, "%s	%d	%d	%d	%ld	%f\n", 
                    get_function_name(function_id), 
                    frame_number, 
                    partition_number,
                    data_reuse,
                    AiecEA_prof_data[function_id].calling_times[frame_number][partition_number][data_reuse], 
                    AiecEA_prof_data[function_id].avg_duration[frame_number][partition_number][data_reuse]      
               );
            }
         }
      }
...
void profile_start(){
   ...
   for(function_id = 0; function_id < NUM_OF_FUNCTIONS; function_id++){
      for(frame_number = 0; frame_number < FRAME_NUM; frame_number++){
         for(partition_number=0; partition_number<PARTITIONS_MAX; partition_number++){
            AiecEA_prof_data[function_id].valid[frame_number][partition_number][0] = false;
            AiecEA_prof_data[function_id].valid[frame_number][partition_number][1] = false;
            AiecEA_prof_data[function_id].total_duration[frame_number][partition_number][0] = 0.0;
            AiecEA_prof_data[function_id].total_duration[frame_number][partition_number][1] = 0.0;
            AiecEA_prof_data[function_id].avg_duration[frame_number][partition_number][0] = 0.0;
            AiecEA_prof_data[function_id].avg_duration[frame_number][partition_number][1] = 0.0;
            AiecEA_prof_data[function_id].calling_times[frame_number][partition_number][0] = 0;
            AiecEA_prof_data[function_id].calling_times[frame_number][partition_number][1] = 0;
         }
      }
   }
}
...
void start_timer(char* function_name, uint32_t frame_number, uint32_t partition_number, uint32_t data_reuse){
   ...
   AiecEA_prof_data[function_id].start_time = now_usec();
}

void stop_timer(char* function_name, uint32_t frame_number, uint32_t partition_number, uint32_t data_reuse){
   uint32_t function_id = get_function_id(function_name);
   AiecEA_prof_data[function_id].total_duration[frame_number][partition_number][data_reuse] += 
                          now_usec() - AiecEA_prof_data[function_id].start_time;
   AiecEA_prof_data[function_id].calling_times[frame_number][partition_number][data_reuse]++;
   AiecEA_prof_data[function_id].avg_duration[frame_number][partition_number][data_reuse] =
                     AiecEA_prof_data[function_id].total_duration[frame_number][partition_number][data_reuse]/
                      ((double)(AiecEA_prof_data[function_id].calling_times[frame_number][partition_number][data_reuse]));
   AiecEA_prof_data[function_id].valid[frame_number][partition_number][data_reuse] = true;
}
